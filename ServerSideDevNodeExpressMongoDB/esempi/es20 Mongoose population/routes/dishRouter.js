...
dishRouter.route('/')
	...
	.get((req, res, next) => {
		Dishes.find({})
		.populate("comments.author") // popola l'autore di ogni commento al piatto
		.then((dishes) => {
	...
	// con Body (raw/json) SENZA commenti; es:
	// {"name": "Uthappizza","image": "images/uthappizza.png","category": "mains","label": "Hot","price": "4.99","featured": "true","description": "A unique combination of Indian Uthappam (pancake) and Italian pizza, topped with Cerignola olives, ripe vine cherry tomatoes, Vidalia onion, Guntur chillies and Buffalo Paneer.","comments": []}
	// e Header: Authorization; value: "bearer [token]"
	...
	// delete all dishes
	// con Header: Authorization; value: "bearer [token]"
...
dishRouter.route('/:dishId/comments')
	...
	// con: Body (raw/json):  {"rating": 3, "comment": "Inserted comment!"}
	// e Header: Authorization; value: "bearer [token]"
	.post(authenticate.verifyUser, (req, res, next) => {
		Dishes.findById(req.params.dishId)
		.then((dish) => {
			if (dish != null) {
				req.body.author = req.user._id; // user è inizializzato da authenticate.verifyUser
				dish.comments.push(req.body);
				dish.save()  // salva l'id dell'autore del commento
				.then((dish) => {
					Dishes.findById(dish._id)
					.populate("comments.author") // ... ma restituisce al client le tutte info dell'autore
					.then((dish) =>{
						res.statusCode = cost.statusOK;
						res.setHeader('Content-Type', 'application/json');
						res.json(dish);
	...
	.put(authenticate.verifyUser, (req, res, next) => {
		...
		.then((dish) => {
			if ((dish != null) && (dish.comments.id(req.params.commentId) != null)) {
				...
				.then((dish) => {
					Dishes.findById(dish._id)
					.populate("comments.author") // popola l'autore del commento al piatto
					.then((dish) => {
						res.statusCode = cost.statusOK;
						res.setHeader('Content-Type', 'application/json');
						res.json(dish);

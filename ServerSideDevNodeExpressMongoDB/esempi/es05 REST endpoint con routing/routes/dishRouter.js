...
dishRouter.route('/')
	...
	.get((req, res, next) => {
		console.log("baseUrl: "+req.baseUrl+";  originalUrl: "+req.originalUrl+";  url: "+req.url)
		res.statusCode = statusOK;
		res.end('Will send all the dishes to you!');
	})
	// POST (Create) localhost:8080/dishes
	// con (in Postman) Body (raw/json): {"name": "pasta", "description": "piatto di pasta"}
	.post((req, res, next) => {
...
dishRouter.route('/:dishId')
	...
	.put((req, res, next) => {
		res.statusCode = statusOK;
		res.end('Updating dish ' + req.params.dishId + " with (new) values; name: " + req.body.name + ' with details: ' + req.body.description);
	})
